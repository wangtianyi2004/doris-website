...
2. 提交导入任务
    ...
    <Tabs>
      <TabItem value="Json 格式数据" label="Json 格式数据" default>
        <div>
        在 Kafka 中，样本数据如下：
      ...
      <TabItem value="Debezium 采集数据" label="Debezium 采集数据" default>
        <div>
        通过 Debezium 采集到的 MySQL 表数据在 `mysql_debezium.test.test_user` Topic 中：
        ...
        <TabItem value="AVRO 序列化格式数据" label="AVRO 序列化格式数据" default>
        <div>
      ...
      <TabItem value="Protobuf 序列化格式数据" label="Protobuf 序列化格式数据" default>
        <div>
        在部署 Kafka Connect 的机器上，通过 curl 命令提交如下导入任务：
        ```Bash
        curl -i http://127.0.0.1:8083/connectors -H "Content-Type: application/json" -X POST -d '{ 
