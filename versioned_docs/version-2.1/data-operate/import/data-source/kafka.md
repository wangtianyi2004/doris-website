...
- Supported message formats are CSV and JSON. Each CSV message is one line, and the line does not contain a newline character at the end;
...
1. Create the table to be loaded:
...
2. Submit the import task:


    <Tabs>
      <TabItem value="JSON Format Data" label="JSON Format Data" default>
        <div>
        The sample data in Kafka is as follows:
        ...
        On the machine where Kafka Connect is deployed, submit the following import task using the curl command:
      ...
      <TabItem value="Debezium Captured Data" label="Debezium Captured Data" default>
        <div>
        The MySQL table data captured by Debezium is in the `mysql_debezium.test.test_user` topic:
        ...
        <TabItem value="AVRO Serialized Data" label="AVRO Serialized Data" default>
        <div>
      ...
      <TabItem value="Protobuf Serialized Data" label="Protobuf Serialized Data" default>
        <div>
        On the machine where Kafka Connect is deployed, submit the following import task using the `curl` command:
        ```Bash
        curl -i http://127.0.0.1:8083/connectors -H "Content-Type: application/json" -X POST -d '{ 
